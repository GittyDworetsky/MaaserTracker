...
const ManageSourcesPage = () => {
  ...
  const handleAddEdit = async () => {
    if (editingSource) {
      await axios.post("/api/maaser/updatesource", { id: editingSource.id, source: selectedSource });
      await getSources();
    } else {
      await axios.post("/api/maaser/addsource", { source: selectedSource });
      await getSources();
    }
    handleClose();
  };
  ...
  const handleDelete = async ({id}) => {
    await axios.post("/api/maaser/deletesource", { id });
    await getSources();
  };
  ...
  return (
    <Container>
          ...
          <TableBody>
            {sources.map((source) => (
              <TableRow key={source.id}>
                <TableCell sx={{ fontSize: '18px' }}>{source.source}</TableCell>
      ...
      <Dialog open={open} onClose={handleClose} fullWidth maxWidth="md">
        ...
        <DialogContent>
          <TextField autoFocus margin="dense" label="Source" type="text" fullWidth value={selectedSource.source} onChange={(e) => setSelectedSource(e.target.value)} />
        </DialogContent>
        <DialogActions>
