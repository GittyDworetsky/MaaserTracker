...
const MaaserPage = () => {
  ...
  useEffect(() => {
    ...
    const getMaaserPayments = async () => {
      const { data } = await axios.get("/api/maaser/getallmaaserdeposits");
          ...
          <TableBody>
            {maaserPayments.map((payment) => (
              <TableRow key={payment.id}>
                ...
                <TableCell align="right" sx={{ fontSize: '18px' }}>${(payment.amount).toFixed(2)}</TableCell>
                <TableCell align="right" sx={{ fontSize: '18px' }}>{dayjs(payment.date).format('DD-MM-YYYY')}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
